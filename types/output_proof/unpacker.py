...
from rollup_bridge_indexer.types.output_proof.decoder import decode
...
class BaseBinarySchema:
    ...
    def __init__(self, value: bytes):
        ...
        self._unpacked: OrderedDict[str, Any] = OrderedDict()
        self._size: int = 0
    ...
    def _import(type_name):
        for module_name in [
            'rollup_bridge_indexer.types.output_proof.inode_tree',
            'rollup_bridge_indexer.types.output_proof.tree_encoding',
            'rollup_bridge_indexer.types.output_proof.x_n',
            'rollup_bridge_indexer.types.output_proof.micheline_expression',
            'rollup_bridge_indexer.types.output_proof.primitive',
            'rollup_bridge_indexer.types.output_proof.output_proof',
            __name__,
        ]:
    ...
    def unpack(self) -> tuple[OrderedDict, int]:
        ...
        for raw in self._schema:
            ...
            if part.size:
                packed_part = self._packed[: cast('int', part.size)]
            else:
            ...
            if isinstance(part.type, str) and part.type[0].islower():
                ...
                part_schema = cast('type[BaseBinarySchema]', part.type)(value=packed_part)
